...
> The aggregate figures in this cable (N=764, 71.2% resilience, 220 gaps) cannot be reproduced
> from any persisted run state and match the retired synthesizer default constants exactly
> (764 / 220 / 0.712). They are not measurements. Retract the headline figures pending
> regeneration; see `ERRATA-2026-09-10.md`. This notice supersedes the claim above that "the
...
* **Analytic Judgment:** Against the mutation vocabulary implemented in `tools/swarm/craftsmen/`, baseline single-point detections held at **71.2%** across 764 probes. Confidence in this figure as a description of the harness is **high**; confidence in it as a predictor of field performance is **low**, because the adversary and the defender share an author.
...
* **Analytic Confidence Level:** **MODERATE**, and capped there by construction. Sample size and safety-gate integrity (764 / 764 Critic approval, deterministic verification, cross-backend SIEM translation) support internal validity. Nothing here establishes external validity, and no volume of self-play can.
