"""Behavioral regression specifications from the 2026-09-06 review.

Permanently integrated into the test suite to ensure no regressions of
confirmed release blockers R02, R03, R06, R07, R08, R10, R11, and R12.
"""
...
def pkg(name: str, capabilities: list[str]) -> PackageCapabilities:
...
def event(seconds: int, host: str, marker: str) -> TelemetryEvent:
...
class ReviewRegressionTests(unittest.TestCase):
    def test_removal_recommendation_actually_breaks_every_chain(self):
        packages = [
            pkg("A", ["fs_read", "web_fetch", "net_egress"]),
            pkg("B", ["fs_read"]),
            pkg("C", ["web_fetch", "net_egress"]),
        ]
    ...
    def test_npm_range_is_not_exactly_pinned(self):
        findings, _ = audit_config_data(
            {"mcpServers": {"x": {"command": "npx", "args": ["demo@^1.0.0", "--ignore-scripts"]}}},
            [],
            {},
        )
    ...
    def test_watchdog_accepts_acquisition_manifest_shape(self):
        ...
        fields = {
            "bin": {"x": "x.js"},
            "scripts": {"postinstall": "node inert.js"},
    ...
    def test_correlation_respects_group_by(self):
        stages = [
            CorrelationStage("a", rule_yaml=rule("a")),
            CorrelationStage("b", rule_yaml=rule("b")),
        ]
