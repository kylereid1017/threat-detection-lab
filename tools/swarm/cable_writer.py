...
_INDEX_TITLE = "# Threat Intelligence Cables Index"
_INDEX_INTRO = (
    "Catalog of structured intelligence cables generated by the detection boundary "
    "harness, adhering to Sherman Kent doctrine and ICD 203 standards."
)
_INDEX_TABLE_HEADER = (
...
def _existing_index_rows(index_path: Path) -> List[str]:
    """Returns the cable rows already present in INDEX.md."""
    rows: List[str] = []
...
def _rebuild_index(index_path: Path, rows: List[str]) -> str:
    """Rebuilds INDEX.md, regenerating only the table rows.

    Every other line already in the file is preserved verbatim - in particular the
    2026-09-10 retraction caution banner, which a naive full rewrite silently deleted.
    """
    preserved: List[str] = []
    if index_path.exists():
        preserved = [
...
"""
    ...
    def _update_index(self, cable_id: str, date_str: str, finding: BoundaryFinding, filename: str) -> None:
        ...
        Only the table rows are regenerated; every other line already in the file
        (notably the retraction caution banner) is preserved.
        """
        index_path = self.cables_dir / "INDEX.md"
        rows = _existing_index_rows(index_path)

        # Add new row if not present
        new_row = f"| [{cable_id}]({filename}) | {date_str} | `{finding.target_rule}` | `{finding.axis}` | `{finding.mutation_name}` | `RESOLVED (patch verified against fixtures)` | [Read Cable]({filename}) |"
        if not any(cable_id in r for r in rows):
            rows.append(new_row)

        index_path.write_text(_rebuild_index(index_path, rows), encoding="utf-8", newline="\n")
